...
export default function PlanDetailPage({
  ...
  const related = PLANS.filter(
    (p) => p.goal === plan.goal && p.id !== plan.id
  ).slice(0, 3);

  // Sample testimonials for this plan
  const planTestimonials = TESTIMONIALS.filter(
    (t) => t.plan === plan.title
  ).slice(0, 2);

  const lvl = levelStyle[plan.level];
    ...
    <div className="min-h-screen">
      ...
      <div className="max-w-6xl mx-auto px-8 py-14">
        <div className="grid lg:grid-cols-3 gap-10">
              ...
              <div className="relative rounded-xl overflow-hidden aspect-video bg-[#111] border border-white/10">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-6">
                  ...
                  <button onClick={handleGetPlan}
                    className="btn btn-white py-2.5 px-7 text-[10px]">
                    Unlock Now
                  </button>
                </div>
              </div>
          ...
          {/* Right — sticky purchase panel (1/3) */}
                  ...
                  <motion.div whileHover={{ y: -4 }}
                    ...
                    <div className="relative h-44 overflow-hidden rounded-t-2xl">
                      <Image src={r.image} alt={r.title} fill
                        ...
                        sizes="400px" />
                      <div className="absolute inset-0 bg-black/30" />
                      <span className={`absolute top-3 left-3 text-[9px] font-bold tracking-widest uppercase px-2.5 py-1 rounded-full ${levelStyle[r.level].bg} ${levelStyle[r.level].text}`}>
                        {r.level}
                      </span>
                    </div>
                    <div className="p-4">
